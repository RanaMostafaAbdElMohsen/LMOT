...
def hungarian_algo(dist, tracks, dets):
    matched_indices = hungarian(dist)
...
def eucleadian_distance(atracks, btracks):
    tracks = np.asarray([track.xy for track in atracks]).reshape(-1,2)
    dets = np.asarray([det.xy for det in btracks]).reshape(-1,2)

    dists = cdist(tracks, dets)
