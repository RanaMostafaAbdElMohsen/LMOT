...
def GSInterpolation(path_in, path_out, interval, tau):
    for (_, _, filenames) in walk(path_in):
        for filename in filenames:
            input_ = np.loadtxt(path_in + "/" + filename, delimiter=',')
            li = LinearInterpolation(input_, interval)
            gsi = GaussianSmooth(li, tau)
            np.savetxt(path_out + "/" + filename, gsi, fmt='%d,%d,%.2f,%.2f,%.2f,%.2f,%.2f,%d,%d,%d')


if __name__ == '__main__':
    mot_val_dir='/home/rana/Documents/Research_Projects/CenterTrack/Masters_Thesis/exp/tracking/72.2/results_mot17halfval'
    mot_val_gsi_dir = '/home/rana/Documents/Research_Projects/CenterTrack/Masters_Thesis/exp/tracking/72.2/results_mot17halfval_gsi'
    GSInterpolation(mot_val_dir, mot_val_gsi_dir, 10, 10)