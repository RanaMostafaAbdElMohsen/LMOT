...
class LinearBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.LeakyReLU, norm_layer=nn.LayerNorm, num_tokens=197):
        super().__init__()
...
class LinearVisionTransformer(nn.Module):
    ...
    def __init__(self, *, patch_size=3, in_chans=7, num_classes=1000, embed_dim=768, depth=12,
                 num_heads=12, mlp_ratio=2., qkv_bias=False, qk_scale=None, drop_rate=0.01, attn_drop_rate=0.,
                 drop_path_rate=0., hybrid_backbone=None, norm_layer=nn.LayerNorm,
                 positional_encoding='learned', learned_positional_encoding_size=(14, 14),
                 block_cls=LinearBlock, num_tokens= 2041, output_size= 2040):
        super().__init__()
        ...
        # Classifier head
        self.head = nn.Linear(num_tokens, output_size) if num_classes > 0 else nn.Identity()
...
@register_model
def linear_tiny(tokens_number, channels, output_size, patch_size=32, pretrained=False, **kwargs):
    model = LinearVisionTransformer(
        patch_size=patch_size, in_chans=channels, num_tokens=tokens_number,  embed_dim=64, 
        output_size=output_size, depth=6, num_heads=3, mlp_ratio=4, qkv_bias=True,
...
#     # Test
#     # x = torch.randn(2, 7, 544, 960)
#     # x= torch.randn(2, 64, 68, 120)
#     x= torch.randn(2, 64, 136, 240)
#     # m = linear_tiny(121, 64, 68*120, 8)
#     # m = linear_tiny(511, 7, 2040)
#     m = linear_tiny(121, 64, 136*240 , 16)
#     out = m(x)
#     print('-----')
